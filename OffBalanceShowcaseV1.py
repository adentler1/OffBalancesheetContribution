...
class OffBalanceShowcase:
    ...
    class Graphs:
        ...
        def tornado(
            ...
            items,                                           # list[dict] with keys:
                                                             ...
                                                             #   optional:  "color": str
                                                             #              "end_text": str (label to draw at bar end)
                                                             #              "end_text_color": str
            title: str = "",
            subtitle: str = "",
            xlabel: str | None = None,
            symmetric: bool = True,
            figsize: tuple[float, float] | None = None,
            show_values: bool = False,                       # backward-compat: show numeric value when no end_text
            value_fmt: str = "{:+.2f}",                      # used only if show_values=True and no end_text
        ):
            """
            Tornado chart:
              - Centered 'name' label at x=0 (e.g., club).
              - Horizontal bar length from 'value' (positive → right, negative → left).
              - End label:
                  * if 'end_text' provided in item → draw that at bar end.
                  * elif show_values=True → draw formatted numeric value at bar end.
                  * else → no end label.
            """
            ...
            # x-limits (pad for end labels)
            if symmetric:
                lim_data = max(abs(min(vals) if vals else 0.0), abs(max(vals) if vals else 0.0))
                lim_data = lim_data if lim_data > 0 else 1.0
                span = 2.0 * lim_data
                extra = 0.10 * span  # slightly more room than before to fit arbitrary text
                ax.set_xlim(-lim_data - extra, lim_data + extra)
            else:
            ...
            # end labels: prefer 'end_text'; fall back to numeric if show_values
            for i, it in enumerate(items):
                v = float(it["value"])
                end_txt = it.get("end_text", None)
                if end_txt is None and show_values:
                    try:
                        end_txt = value_fmt.format(v)
                    except Exception:
                        end_txt = f"{v:+.2f}"
                if not end_txt:
                    continue
                ...
                ax.text(
                    x_target, i, str(end_txt),
                    ha=ha, va="center",
                    fontsize=self.s.tick_fontsize,
                    color=it.get("end_text_color", "black"),
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.90),
        ...
        def ranking(
            ...
            figsize: tuple[float, float] | None = None,
            *,
            end_labels: list[str] | None = None,  # optional per-bar strings at the bar end
            colors: list[str] | None = None,      # optional per-bar colors
            xlabel: str = "Off-Balance-Sheet Contribution",
            show_zero_line: bool = True,
        ):
            """
            Horizontal ranking bars with optional text at the 'bar end'.
            - end_labels[i] (if provided) is placed outside the bar:
                right of positive bars, left of negative bars.
            - If values contain negatives, we pad both sides so labels don’t clip.
            """
            ...
            fig, ax = self._new_fig_ax(title, subtitle, figsize)
            ax.barh(cats, vals, color=colors, zorder=2)
        
            if show_zero_line:
                ax.axvline(0, color=self.s.colors.get("line", "black"), lw=0.8, zorder=1)
        
            # x-limits with room for end labels if present
            xmin = float(np.nanmin(vals)) if vals.size else 0.0
            xmax = float(np.nanmax(vals)) if vals.size else 1.0
            if xmax <= xmin:
                xmax = xmin + 1.0
            span = xmax - xmin
            pad_left = pad_right = 0.06 * span
            if end_labels:
                # give more margin if we will draw end labels
                pad_left  += 0.08 * span
                pad_right += 0.08 * span
            ...
            if end_labels:
                ...
                span = x1 - x0
                dx = 0.02 * span
                for yi, (v, txt) in enumerate(zip(vals, end_labels)):
        ...
        def toplists_table(
            self,
            columns,                     # list of {"header": str, "items": list[tuple[str, str]]}
            *,
            title: str = "",
            subtitle: str = "",
            top_n: int = 5,
            ...
            """
            Render a simple 3-column 'top lists' table as a plot.
        
            columns: e.g.
              [
                {"header": "Top 5 Scorers",       "items": [("Player A","30"), ("B","28"), ...]},
                {"header": "Top 5 Assists",       "items": [("Player C","12"), ...]},
                {"header": "Top 5 Contributors",  "items": [("Player D","+0.42"), ...]},
              ]
            """
        
            # --- normalize: ensure each column has <= top_n items ---
            cols = []
            for col in columns:
                hdr = str(col.get("header", ""))
                items = list(col.get("items", []))[:int(top_n)]
                cols.append({"header": hdr, "items": items})
            ncols = len(cols)
            if ncols == 0:
                raise ValueError("toplists_table: no columns provided.")
        
            # --- figure / axes (standardized) ---
            fig, ax = self._new_fig_ax(title, subtitle, figsize)
            ax.axis("off")
        
            # --- build cell matrix for matplotlib.table ---
            # Rows = top_n, Cols = ncols; each cell is "rank. Name — value"
            headers = [c["header"] for c in cols]
            max_rows = max((len(c["items"]) for c in cols), default=0)
            max_rows = min(max_rows, int(top_n))
            rows_matrix = []
            for r in range(max_rows):
                ...
                for c in cols:
                    if r < len(c["items"]):
                        name, val = c["items"][r]
                        row_cells.append(f"{r+1}. {name} — {val}")
                    else:
                        row_cells.append("")
                rows_matrix.append(row_cells)
        
            # --- draw a table occupying the center area (leave footer space below) ---
            table = ax.table(
                cellText=rows_matrix,
                cellLoc="left",
                colLabels=headers,
                colLoc="left",
                loc="center",
                bbox=[0.04, 0.06, 0.92, 0.82],   # [left, bottom, width, height] w.r.t axes
            )
            table.auto_set_font_size(False)
            table.set_fontsize(self.s.tick_fontsize)
        
            # resize a bit depending on how many rows we show
            scale_y = 1.0 if max_rows <= 6 else 0.90 if max_rows <= 10 else 0.80
            table.scale(1.05, scale_y)
        
            # bold headers
            for (r, c), cell in table.get_celld().items():
                if r == 0 and cell.get_text().get_text() in headers:
    ...
    # ------------------------ AUDIT + CONTRIBUTION HELPERS ------------------------
    
    def load_season_audit(self, path: str = "/mnt/data/season_audit.xlsx") -> None:
        """
        Load season_audit.xlsx once and keep on self.season_audit.
        Expected columns (flexible): country, league, season, available(bool/flag), etc.
        """
        import pandas as pd
        df = pd.read_excel(path)
        # normalize common fields
        cols = {c.lower().strip(): c for c in df.columns}
        # we accept flexible schemas; just store raw but add std view if present
        def getcol(*names):
            for n in names:
                if n in cols: return cols[n]
            return None
        c_country = getcol("country")
        c_league  = getcol("league")
        c_season  = getcol("season")
        if c_season is not None:
            df["_season"] = pd.to_numeric(df[c_season], errors="coerce").astype("Int64")
        if c_country is not None: df["_country"] = df[c_country].astype(str)
        if c_league  is not None: df["_league"]  = df[c_league].astype(str)
        self.season_audit = df
    
    
    def _resolve_contrib_col(self, cols: list[str], override: str | None = None) -> str:
        """
        Choose contribution column:
        1) explicit override argument
        2) self.contribution_col (if provided on the instance)
        3) preference list
        """
        if override and override in cols:
            return override
        if getattr(self, "contribution_col", None) and self.contribution_col in cols:
            return self.contribution_col
        prefs = [
            "contribution_ols",
            "ridge_contribution_alpha_1_0",
            "ridge_contribution_alpha_10_0",
            "lasso_contribution_alpha_0_01",
            "lasso_contribution_alpha_0_001",
            "partialled_contribution_ols",
            "partialled_ridge_contribution_alpha_1_0",
            "partialled_ridge_contribution_alpha_10_0",
            "partialled_lasso_contribution_alpha_0_01",
            "partialled_lasso_contribution_alpha_0_001",
            "impact",
        ]
        s = set(cols)
        for c in prefs:
            if c in s:
                return c
        for c in cols:
            if "contribution" in c.lower():
                return c
        raise ValueError("No contribution column found.")
    
    
    def _fetch_scope_df(self, *, country: str, league: str, season: int):
        import sqlite3, pandas as pd
        with sqlite3.connect(self.db_path) as con:
            q = "SELECT * FROM analysis_results WHERE country=? AND league=? AND season=?"
            df = pd.read_sql_query(q, con, params=[country, league, int(season)])
        return df
    ...
    def mod_team_top_player_tornado(
        self,
        *,
        country: str,
        league: str,
        season: int,
        top_n: int | None = None,              # None → all clubs
        contrib_col: str | None = None,        # auto-pick if None
        append_value_to_label: bool = False,   # e.g., "X. Player (+0.42)"
        exclude_multiple_teams: bool = True,
        symmetric: bool = True,
    ):
        """
        For each club in a league-season, find the player with the highest contribution
        and draw a tornado: club centered at zero, bar to left/right by contribution,
        end label = player name (optionally with value).
        """
        ...
        if df.empty:
            raise ValueError("No data for chosen scope.")
    
        # Clean / filter
        df["player_name"] = df["player_name"].fillna("").astype(str)
        df = df[~df["player_name"].str.strip().str.lower().isin(["", "unknown"])]
    
        if exclude_multiple_teams and "team(s)" in df.columns:
            df = df[~df["team(s)"].astype(str).str.startswith("Multiple Teams")]
    
        # pick contribution column if not provided
        def _pick_contrib_col(cols: list[str]) -> str:
            prefs = [
                "contribution_ols",
                "ridge_contribution_alpha_1_0",
                "ridge_contribution_alpha_10_0",
        ...
        metric = contrib_col or _pick_contrib_col(df.columns.tolist())
        df[metric] = pd.to_numeric(df[metric], errors="coerce")
    
        # per-club top player
        club_col = "team(s)" if "team(s)" in df.columns else "team_name"
        if club_col not in df.columns:
            raise ValueError("No club column found (expected 'team(s)' or 'team_name').")
    
        # pick the row with max contribution per club
        idx = df.groupby(club_col, as_index=False)[metric].idxmax().dropna()[metric].astype(int).tolist()
        top_rows = df.loc[idx].copy()
        ...
        for _, r in top_rows.iterrows():
            ...
            end_txt = f"{player} ({v:+.2f})" if append_value_to_label else player
            items.append({
                "name": nm,
                "value": v,
                "color": self.style.colors["top"] if v >= 0 else self.style.colors["bottom"],
                "end_text": end_txt,
            })
    
        # sort (largest to smallest) and optionally trim
        items.sort(key=lambda d: d["value"], reverse=True)
        if top_n is not None:
            items = items[:int(top_n)]
    
        title = f"{country} / {league} / {season}: Top Contributor per Club"
        subtitle = "Bar direction = contribution sign. Center = club, end label = top player."
        fig = self.graphs.tornado(
            items,
        ...
        return self._finish(fig, "tornado_club_top_player")

    def mod_team_ranking(
        self,
        *,
        country: str,
        league: str,
        season: int,
        contrib_col: str | None = None,
        exclude_multiple_teams: bool = True,
        sort_ascending: bool = False,        # False → best at top
        annotate_top_player: bool = True,
    ):
        """
        Ranking bar chart: team average contribution, with optional end label = top player per team.
        """
        import sqlite3, pandas as pd, numpy as np
        ...
        # team average
        g = df.groupby(club_col, as_index=False)[metric].mean().rename(columns={metric: "avg_contrib"})
        # order
        g = g.sort_values("avg_contrib", ascending=bool(sort_ascending)).reset_index(drop=True)
    
        # end labels (top player per team)
        end_labels = None
        if annotate_top_player:
            idx = df.groupby(club_col, as_index=False)[metric].idxmax().dropna()[metric].astype(int).tolist()
            tops = df.loc[idx, [club_col, "player_name", metric]]
            tops = tops.rename(columns={"player_name": "top_player", metric: "top_val"})
            g = g.merge(tops, on=club_col, how="left")
            end_labels = (g["top_player"].fillna("").astype(str)).tolist()
    
        # colors: sign-aware optional (else uniform)
        colors = [self.style.colors["top"] if v >= 0 else self.style.colors["bottom"] for v in g["avg_contrib"].tolist()]
    
        title = f"{country} / {league} / {season}: Team Average Contribution"
        subtitle = "End label = team’s top contributor (by same metric)."
        fig = self.graphs.ranking(
            categories=g[club_col].tolist(),
            values=g["avg_contrib"].to_numpy(dtype=float),
            title=title, subtitle=subtitle,
            end_labels=end_labels,
            colors=colors,
            xlabel="Average Contribution",
            show_zero_line=True
        )
        return self._finish(fig, "ranking_team_avg_with_labels")
    ...
    def mod_top5_lists(
        ...
        *,
        ...
        season: int | None = None,
        top_n: int = 5,
        contrib_col: str | None = None,   # if None -> auto-pick from df
        min_minutes: float = 0.0,         # optional filter
    ):
        """
        Produce a 3-column 'Top 5' list: Scorers, Assists, Contributors (plot).
        Scope can be passed here or taken from self.country/league/season if set.
        Data source: analysis_results (self.db_path).
        """
        import sqlite3
        import pandas as pd
        import numpy as np
        ...
        season  = int(season if season is not None else (self.season if self.season is not None else -1))
        if not country or not league or season < 0:
            raise ValueError("mod_top5_lists needs country, league, season (either passed or set on the instance).")
    
        # --- load scope from analysis_results ---
        with sqlite3.connect(self.db_path) as con:
            q = "SELECT * FROM analysis_results WHERE country=? AND league=? AND season=?"
            df = pd.read_sql_query(q, con, params=[country, league, season])
    
        if df.empty:
            raise ValueError(f"No rows in analysis_results for {country}/{league}/{season}.")
    
        # --- clean names, optional minutes filter ---
        df["player_name"] = df["player_name"].fillna("").astype(str)
        df = df[~df["player_name"].str.strip().str.lower().isin(["", "unknown"])]
        if min_minutes > 0 and "minutes_played" in df.columns:
            df = df[pd.to_numeric(df["minutes_played"], errors="coerce").fillna(0.0) >= float(min_minutes)]
        if df.empty:
            raise ValueError("No eligible players after filtering.")
        ...
        def _pick_contrib_col(cols: list[str]) -> str:
            prefs = [
                ...
                "partialled_lasso_contribution_alpha_0_01",
                "partialled_lasso_contribution_alpha_0_001",
                "impact",  # ultimate fallback
            ]
            s = set(cols)
            for c in prefs:
                if c in s:
                    return c
            # last resort: take any column containing 'contribution'
            for c in cols:
                if "contribution" in c.lower():
                    return c
            raise ValueError("No contribution column found.")
        ...
        if contrib_col is None:
            contrib_col = _pick_contrib_col(df.columns.tolist())
    
        # --- numeric coercions ---
        for col in ["goals", "assists", contrib_col]:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce")
        # Keep one row per player (season scope usually already is)
        # prefer player_id for uniqueness if present
        key = "player_id" if "player_id" in df.columns and df["player_id"].notna().any() else "player_name"
    
        # --- build top-N lists ---
        def _top_list(metric: str, fmt: str = "int"):
            if metric not in df.columns:
                return []
            tmp = df[[key, "player_name", metric]].dropna(subset=[metric]).copy()
            tmp = tmp.sort_values(metric, ascending=False)
            tmp = tmp.head(int(top_n))
            items = []
            for _, r in tmp.iterrows():
                ...
                if fmt == "int":
                    ...
                    # contribution formatting: sign + 2 decimals
                    try:
                        s = f"{val:+.2f}"
                    except Exception:
                        s = f"{float(val):+.2f}"
        ...
        top_scorers  = _top_list("goals",   fmt="int") if "goals"   in df.columns else []
        top_assists  = _top_list("assists", fmt="int") if "assists" in df.columns else []
        top_contribs = _top_list(contrib_col, fmt="cont")
    
        # ensure we have columns in desired order, filling blanks if missing
        columns = [
            {"header": f"Top {top_n} Scorers",      "items": top_scorers},
            {"header": f"Top {top_n} Assists",      "items": top_assists},
            {"header": f"Top {top_n} Contributors", "items": top_contribs},
        ]
    
        title = f"{country} / {league} / {season}: Top {top_n} Lists"
        subtitle = f"Contributors by '{contrib_col}'"
    
        fig = self.graphs.toplists_table(columns, title=title, subtitle=subtitle, top_n=top_n)
        return self._finish(fig, f"top{top_n}_lists")

     
    # ------------------------ Q1: EXTENDED TOP LISTS + WINNERS/LOSERS  ------------------------
    
    def mod_toplists_extended(
        self,
        *,
        audit_df: pd.DataFrame | None = None,       # pass self.season_audit or a filtered slice
        country: str | None = None,
        league: str | None = None,
        season: int | None = None,
        top_n: int = 5,
        contrib_col: str | None = None,
        include_overlap: bool = True,               # overlap of top contributors with top goals/assists
        include_winners_losers: bool = True,        # top contributors among winners & losers
        include_most_improved: bool = True,         # teams improving rank the most (needs prior season)
        min_minutes: float = 0.0,
    ):
        """
        One figure with 3–4 narrow 'top lists' tables stitched as separate figures (call x3/4),
        OR call multiple times and post as a series.
        This function produces a composite 'Top Lists' that includes:
          - Top contributors (overall)
          - Overlap with top scorers / assists (if requested and available)
          - Top contributors among season winners and season losers (bottom teams)
          - Top contributors for teams that improved ranking the most vs prior season
        """
        import numpy as np, pandas as pd
    
        # Scope resolution via audit or direct parameters
        if audit_df is not None:
            # pick first row as main scope if explicit scope not provided
            sub = audit_df
            if country: sub = sub[sub["_country"].astype(str) == str(country)]
            if league:  sub = sub[sub["_league"].astype(str) == str(league)]
            if season is not None: sub = sub[sub["_season"] == int(season)]
            if sub.empty:
                raise ValueError("Audit has no matching scope for the requested filters.")
            # pick one scope (or you can extend to loop over many)
            country = country or sub["_country"].iloc[0]
            league  = league  or sub["_league"].iloc[0]
            season  = int(season if season is not None else sub["_season"].iloc[0])
    
        if not country or not league or season is None:
            raise ValueError("Specify country, league, season or pass an audit_df with those columns.")
    
        # Load scope df
        df = self._fetch_scope_df(country=country, league=league, season=season)
        if df.empty:
            raise ValueError("No data for chosen scope.")
        df["player_name"] = df["player_name"].fillna("").astype(str)
        df = df[~df["player_name"].str.strip().str.lower().isin(["", "unknown"])]
        ...
        metric = self._resolve_contrib_col(df.columns.tolist(), contrib_col)
        df[metric] = pd.to_numeric(df[metric], errors="coerce")
    
        # ---- A) Overall Top Contributors (table) ----
        def _mk_table(items, header):
            return {"header": header, "items": [(n, v) for n, v in items]}
    
        def _top(df_, col, fmt="cont"):
            tmp = df_.dropna(subset=[col]).copy()
            tmp = tmp.sort_values(col, ascending=False).head(int(top_n))
            out = []
            for _, r in tmp.iterrows():
                name = str(r["player_name"])
                val  = r[col]
                if fmt == "int":
                    s = f"{int(val)}" if pd.notnull(val) else ""
                else:
                    s = f"{float(val):+.2f}" if pd.notnull(val) else ""
                out.append((name, s))
            return out
    
        cols = []
        cols.append(_mk_table(_top(df, metric, fmt="cont"), f"Top {top_n} Contributors ({metric})"))
    
        # ---- B) Overlap with goals / assists ----
        if include_overlap:
            if "goals" in df.columns:
                cols.append(_mk_table(_top(df, "goals", "int"), f"Top {top_n} Scorers"))
            if "assists" in df.columns:
                cols.append(_mk_table(_top(df, "assists", "int"), f"Top {top_n} Assists"))
    
        # ---- C) Winners & Losers (top contributors inside subsets) ----
        if include_winners_losers and "league_position" in df.columns:
            # Winner(s): league_position == 1
            winners = df[pd.to_numeric(df["league_position"], errors="coerce") == 1]
            if not winners.empty:
                cols.append(_mk_table(_top(winners, metric, "cont"), f"Winners’ Top {top_n} Contributors"))
            # Losers: bottom-3 (max league_position)
            lp = pd.to_numeric(df["league_position"], errors="coerce")
            if lp.notna().any():
                maxpos = int(lp.max())
                losers = df[lp.isin([maxpos, maxpos-1, maxpos-2])]
                if not losers.empty:
                    cols.append(_mk_table(_top(losers, metric, "cont"), f"Bottom-3 Teams’ Top {top_n} Contributors"))
    
        # ---- D) Most improved teams (compare league_position vs prior season) ----
        if include_most_improved and "league_position" in df.columns:
            # try fetch prior season
            try:
                df_prev = self._fetch_scope_df(country=country, league=league, season=int(season)-1)
                if not df_prev.empty and "league_position" in df_prev.columns:
                    # team average league_position (usually same per team) — we just need team-level change
                    club_col = "team(s)" if "team(s)" in df.columns else "team_name"
                    g_now  = df.dropna(subset=[club_col,"league_position"])[[club_col,"league_position"]].drop_duplicates()
                    g_prev = df_prev.dropna(subset=[club_col,"league_position"])[[club_col,"league_position"]].drop_duplicates()
                    rename = lambda d: d.rename(columns={"league_position":"lp"})
                    g_now, g_prev = rename(g_now), rename(g_prev)
                    imp = g_prev.merge(g_now, on=club_col, how="inner", suffixes=("_prev","_now"))
                    imp["delta"] = imp["lp_prev"] - imp["lp_now"]   # positive = improved
                    imp = imp.sort_values("delta", ascending=False).head(3)   # top improvers
                    # gather top contributors inside those improved teams
                    if not imp.empty:
                        imp_teams = df[df[club_col].isin(imp[club_col])]
                        cols.append(_mk_table(_top(imp_teams, metric, "cont"), "Top Contributors: Most-Improved Teams"))
            except Exception:
                pass
    
        # Render 3–4 columns (truncate to 3 if too many) by calling the table primitive multiple times or once
        # Here we render a wide single table with up to 3 columns; if more, we generate multiple images (return last)
        chunks = [cols[i:i+3] for i in range(0, len(cols), 3)]
        out_last = None
        for idx, chunk in enumerate(chunks, 1):
            fig = self.graphs.toplists_table(
                chunk,
                title=f"{country} / {league} / {season}: Top Lists ({idx}/{len(chunks)})",
                subtitle=f"Contribution metric: '{metric}'",
                top_n=top_n,
            )
            out_last = self._finish(fig, f"toplists_ext_{idx}")
        return out_last
    
    
    # ------------------------ Q2: BOTTOM-3 TEAMS → TOP-3 CONTRIBUTORS ------------------------
    
    def mod_bottom3_top3_contributors_table(
        self, *, country: str, league: str, season: int,
        contrib_col: str | None = None, min_minutes: float = 0.0
    ):
        """
        Find bottom-3 teams by league_position, then list their top-3 contributors in a 3-column table.
        """
        import numpy as np, pandas as pd
        ...
        lp = pd.to_numeric(df["league_position"], errors="coerce")
        if lp.isna().all():
            raise ValueError("league_position is missing; cannot identify bottom-3 teams.")
        maxpos = int(lp.max())
        bottom_ids = [maxpos, maxpos-1, maxpos-2]
    
        tables = []
        for pos in bottom_ids:
            sub = df[lp == pos]
            if sub.empty: continue
            # top-3 contributors inside this set
            top = (sub.sort_values(metric, ascending=False)
                       .head(3)[["player_name", metric, club_col]]
                       .reset_index(drop=True))
            items = [(f"{r['player_name']} ({r[club_col]})", f"{float(r[metric]):+.2f}") for _, r in top.iterrows()]
            tables.append({"header": f"Pos {pos}: Top 3 Contributors", "items": items})
    
        if not tables:
            raise ValueError("Bottom-3 tables are empty.")
    
        fig = self.graphs.toplists_table(tables, title=f"{country} / {league} / {season}: Bottom-3 Teams — Top Contributors",
                                         subtitle=f"Contribution metric: '{metric}'", top_n=3)
        return self._finish(fig, "bottom3_top3_contributors")
    
    
    # ------------------------ Q3: POSITIONS BY MEDIAN (TORNADO) / BUCKETS ------------------------
    
    def mod_position_median_tornado(
        self, *, country: str, league: str, season: int,
        contrib_col: str | None = None, by: str = "position"
    ):
        """
        Use tornado with per-position median contribution (bars left/right around 0).
        """
        import numpy as np, pandas as pd
        ...
        key = by if by in df.columns else "position"
        med = (df.dropna(subset=[key, metric])
                 .groupby(key, as_index=False)[metric].median()
                 .rename(columns={metric:"median"}))
        med = med.sort_values("median", ascending=False).reset_index(drop=True)
        ...
        title = f"{country} / {league} / {season}: Median Contribution by {key.capitalize()}"
        fig = self.graphs.tornado(items, title=title, subtitle="", xlabel="Median Contribution",
                                  symmetric=True, show_values=False)
        return self._finish(fig, "positions_median_tornado")
    
    
    def mod_position_buckets_horizontal_positions(
        self, *, country: str, league: str, season: int,
        contrib_col: str | None = None
    ):
        """
        Per-position distributions using horizontal buckets (IQR band + whiskers/outliers).
        """
        import numpy as np, pandas as pd
        ...
        key = "position"
        items = []
        for pos, sub in df.dropna(subset=[key, metric]).groupby(key):
            vals = pd.to_numeric(sub[metric], errors="coerce").dropna().to_numpy(dtype=float)
            if vals.size == 0: continue
            items.append({"name": str(pos), "values": vals})
        if not items: raise ValueError("No position distributions found.")
    
        title = f"{country} / {league} / {season}: Contribution Distributions by Position"
        fig = self.graphs.bucket_horizontal(items, title=title, subtitle="", xlabel="Contribution",
                                            symmetric=True, showfliers=True)
        return self._finish(fig, "positions_buckets_horizontal")
    
    
    # ------------------------ Q5: CONTRIBUTION vs AGE (with highlight above band) ---------------
    
    def mod_contrib_vs_age_clean(
        self, *, country: str, league: str, season: int,
        contrib_col: str | None = None, min_minutes: float = 0.0
    ):
        """
        Age vs contribution scatter with regression & prediction band; only label players above band.
        """
        import numpy as np, pandas as pd
        ...
        # age column strategy
        agecol = None
        for c in ["age_at_season_start","age"]:
            if c in df.columns:
                agecol = c; break
        if agecol is None:
            raise ValueError("Age column not present. Provide 'age_at_season_start' or 'age' in analysis_results.")
    
        if min_minutes > 0 and "minutes_played" in df.columns:
            df = df[pd.to_numeric(df["minutes_played"], errors="coerce").fillna(0.0) >= float(min_minutes)]
        df = df.dropna(subset=[metric, agecol, "player_name"])
    
        # draw regression with prediction band; highlights are those above band
        # our scatter implements regline; we emulate highlighting by picking top residuals
        x = pd.to_numeric(df[agecol], errors="coerce").to_numpy(dtype=float)
        ...
        xb, yb = x.mean(), y.mean()
        Sxx = np.sum((x-xb)**2)
        b1  = np.sum((x-xb)*(y-yb)) / (Sxx if Sxx>0 else 1)
        b0  = yb - b1*xb
        yhat = b0 + b1*x
        resid = y - yhat
        # choose highlight as top 1–2% largest positive residuals (min 5)
        k = max(5, int(0.02 * len(resid)))
        idx = np.argsort(resid)[-k:]
        highlights = [{"x": float(x[i]), "y": float(y[i]), "text": str(df["player_name"].iloc[np.where(mask)[0][i]]), "side": "right", "color": "red"} for i in idx]
    
        sources = [{"x": x, "y": y, "labels": df["player_name"].iloc[np.where(mask)[0]].to_numpy(), "name": None}]
        title = f"{country} / {league} / {season}: Contribution vs Age"
        subtitle = "Labels limited to players above the prediction line."
        fig = self.graphs.scatter_fte(sources, title=title, subtitle=subtitle,
                                      xlabel="Age", ylabel="Contribution",
                                      hline="zero", regline=True, reg_ci="prediction", highlights=highlights)
        return self._finish(fig, "contrib_vs_age_clean")
    
    
    # ------------------------ Q7: YOUNG DEVELOPERS VIA SECONDARY LEAGUES ------------------------
    
    def mod_young_development_secondary(
        self, *,
        audit_df: pd.DataFrame,                  # pass self.season_audit (filtered if you like)
        main_country: str, main_league: str, main_season: int,
        secondary_filters: dict | None = None,   # e.g., {"country": ["Netherlands","Switzerland"], "tier": ["1"], "league": [...]}
        age_max: int = 22,
        contrib_col: str | None = None,
        max_players: int = 20
    ):
        """
        Identify young players (<= age_max) active in main_league, whose previous season was in 'secondary' leagues,
        and show their slopes (prev → current) of contribution.
        """
        import numpy as np, pandas as pd
    
        # current season df
        df_now = self._fetch_scope_df(country=main_country, league=main_league, season=main_season)
        if df_now.empty: raise ValueError("No data (current).")
        metric = self._resolve_contrib_col(df_now.columns.tolist(), contrib_col)
        df_now[metric] = pd.to_numeric(df_now[metric], errors="coerce")
        df_now["player_name"] = df_now["player_name"].fillna("").astype(str)
        # young filter
        agecol = "age_at_season_start" if "age_at_season_start" in df_now.columns else ("age" if "age" in df_now.columns else None)
        if agecol is None: raise ValueError("No age column for current season.")
        df_now = df_now[pd.to_numeric(df_now[agecol], errors="coerce") <= float(age_max)]
    
        # previous season from audit (require matching player_id or name)
        prev_season = int(main_season) - 1
        # We accept "secondary_filters" as a whitelist of prior leagues (by country/league strings)
        # If absent, default to Netherlands/Switzerland + top-5 second tiers
        if not secondary_filters:
            secondary_filters = {"country": ["Netherlands","Switzerland","Germany","England","Spain","Italy","France"],
                                 "league":    None}  # allow leagues filtered later if needed
    
        # gather previous-season rows from those countries/leagues present in audit
        rows = []
        for _, a in audit_df.iterrows():
            if a.get("_season", None) != prev_season: continue
            cc = str(a.get("_country", "")); lg = str(a.get("_league", ""))
            if secondary_filters.get("country") and cc not in secondary_filters["country"]: continue
            if secondary_filters.get("league") and lg not in secondary_filters["league"]: continue
            try:
                tmp = self._fetch_scope_df(country=cc, league=lg, season=prev_season)
                if not tmp.empty:
                    rows.append(tmp)
            except Exception:
                continue
        if not rows:
            raise ValueError("No prior-season rows found for secondary leagues (given filters).")
        df_prev_all = pd.concat(rows, ignore_index=True)
    
        # join by player_id if possible, else name
        use_id = "player_id" in df_now.columns and "player_id" in df_prev_all.columns
        if use_id:
            keys = ["player_id"]
        else:
            keys = ["player_name"]
    
        df_prev_all[metric] = pd.to_numeric(df_prev_all[metric], errors="coerce")
    
        merged = pd.merge(
            df_now[keys + ["player_name", metric]],
            df_prev_all[keys + [metric]],
            on=keys, suffixes=("_now", "_prev")
        ).dropna(subset=[f"{metric}_prev", f"{metric}_now"])
        if merged.empty:
            raise ValueError("No matched young players with prior-season secondary league.")
    
        # choose a manageable subset
        merged = merged.sort_values(f"{metric}_now", ascending=False).head(int(max_players))
    
        # slope items
        items = []
        for _, r in merged.iterrows():
            items.append({
                "start_value": float(r[f"{metric}_prev"]),
                "final_value": float(r[f"{metric}_now"]),
                "label": str(r["player_name"]),
                "flag_to_write_value": True,
                "color": "tab:blue" if float(r[f"{metric}_now"]) >= float(r[f"{metric}_prev"]) else "red",
                "linewidth": 1.8,
            })
    
        title = f"{main_country} / {main_league} {main_season}: Young Developers via Secondary Leagues"
        subtitle = "Prev season (secondary) → current (main). Δ shown at the end."
        fig = self.graphs.slope(items, title=title, subtitle=subtitle,
                                sort_start_ascending=True, xlabel="Contribution")
        return self._finish(fig, "young_development_secondary")
    
    
    # ------------------------ Q10: DIVISION CLASSIFIER + COMPARISON ------------------------
    
    def mod_division_distributions(
        self, *,
        audit_df: pd.DataFrame,
        season: int,
        divisions_map: dict[str, str],      # mapping league_name -> "Top" / "Second" / "Other"
        leagues: list[str] | None = None,
        contrib_col: str | None = None
    ):
        """
        Compare contribution distributions across divisions (Top vs Second vs Other)
        using horizontal buckets. 'divisions_map' classifies leagues.
        """
        import numpy as np, pandas as pd
        ...
        groups = {}
        for _, a in audit_df.iterrows():
            if a.get("_season", None) != int(season): continue
            lg = str(a.get("_league","")); cc = str(a.get("_country",""))
            if leagues and lg not in leagues: continue
            tag = divisions_map.get(lg, "Other")
            try:
                df = self._fetch_scope_df(country=cc, league=lg, season=season)
                if df.empty: continue
                metric = self._resolve_contrib_col(df.columns.tolist(), contrib_col)
                vals = pd.to_numeric(df[metric], errors="coerce").dropna().to_numpy(dtype=float)
                if vals.size == 0: continue
                groups.setdefault(tag, []).extend(vals.tolist())
            except Exception:
                continue
    
        if not groups:
            raise ValueError("No distributions assembled for requested season/divisions.")
    
        items = [{"name": tag, "values": np.array(vals, dtype=float)} for tag, vals in groups.items()]
        title = f"{season}: Contribution Distributions by Division"
        fig = self.graphs.bucket_horizontal(items, title=title, subtitle="Top vs. Second vs. Other divisions",
                                            xlabel="Contribution", symmetric=True, showfliers=True)
        return self._finish(fig, "division_distributions")
    
    
    # ------------------------ Q14: NEW vs EXISTING — BY POSITION (GROUPED BUCKETS) -----------
    
    def mod_new_vs_existing_by_position(
        self, *,
        country: str, league: str, season: int,
        contrib_col: str | None = None,
        transfers_db: str = "player_profiles.db",    # transfers stored in same DB in your design
        refresh_active_only: bool = True
    ):
        """
        Compare contribution distributions of newly acquired players vs incumbents, per position.
        New = players with a transfer date within the last window (prev summer/winter) before the season.
        """
        import sqlite3, pandas as pd, numpy as np
        ...
        # mark new vs existing using transfers table: any transfer <= season year window
        news = set()
        try:
            with sqlite3.connect(transfers_db) as con:
                t = pd.read_sql_query("SELECT player_id, transfer_date FROM player_transfers", con)
            t["player_id"] = pd.to_numeric(t["player_id"], errors="coerce")
            t["transfer_date"] = pd.to_datetime(t["transfer_date"], errors="coerce", utc=True)
            # window: last 12 months up to Aug 1 season year (rough heuristic)
            start = pd.Timestamp(datetime.date(int(season)-1, 8, 1), tz="UTC")
            end   = pd.Timestamp(datetime.date(int(season), 8, 1), tz="UTC")
            recent = t[(t["transfer_date"] >= start) & (t["transfer_date"] < end)]
            news = set(recent["player_id"].dropna().astype(int).tolist())
        except Exception:
            pass
    
        df["_group"] = np.where(df.get("player_id").isin(news), "New", "Existing")
        # position buckets
        def _pos_bucket(s: str) -> str:
            p = (s or "").lower()
            if any(k in p for k in ["def","back","cb","rb","lb","df"]): return "Defenders"
            if any(k in p for k in ["mid","cm","dm","am","wing","mf"]): return "Midfielders"
            if any(k in p for k in ["fw","st","cf","striker","forward","attacker"]): return "Strikers"
            return "Other"
        df["_pos"] = df.get("position","").astype(str).map(_pos_bucket)
    
        # Build data_map: top group = position; sub groups = New/Existing
        data_map = {}
        for pos in ["Defenders","Midfielders","Strikers","Other"]:
            sub = df[df["_pos"] == pos]
            if sub.empty: continue
            data_map[pos] = {
                "Existing": pd.to_numeric(sub.loc[sub["_group"]=="Existing", metric], errors="coerce").dropna().tolist(),
                "New":      pd.to_numeric(sub.loc[sub["_group"]=="New",      metric], errors="coerce").dropna().tolist(),
            }
    
        title = f"{country} / {league} / {season}: New vs Existing by Position"
        subtitle = "Boxplots show distributions of contribution per position for new signings vs incumbents."
        fig = self.graphs.bucket_grouped_vertical(
            data_map,
            group_order=list(data_map.keys()),
            sub_order=["Existing","New"],
            title=title, subtitle=subtitle, ylabel="Contribution", show_legend=True, figsize=(12,6)
        )
        return self._finish(fig, "new_vs_existing_by_position")
    
    
    # ------------------------ Q16: LEGENDARY PLAYER COMPARISON -------------------------------
    
    def mod_legendary_player_career_compare(
        self, *,
        country: str, league: str,
        players: list[str],                 # names or IDs; long-tenure comparisons
        seasons: list[int] | None = None,
        use_ids: bool = False,
        contrib_col: str | None = None,
        club_filter: str | None = None,     # e.g., "Bayern Munich" for Müller
        agg: str = "sum"
    ):
        """
        Compare contribution across seasons for a set of legendary/long-tenure players.
        """
        import pandas as pd, numpy as np
    
        if seasons is None:
            # try last 10 seasons in audit (or 8 as default)
            seasons = list(range(datetime.date.today().year-9, datetime.date.today().year+1))
        ...
        for p in players:
            ...
            found_any = False
            for s in seasons:
                df = self._fetch_scope_df(country=country, league=league, season=s)
                if df.empty: 
                ...
                if club_filter:
                    club_col = "team(s)" if "team(s)" in df.columns else "team_name"
                    df = df[df[club_col] == club_filter]
                if use_ids and "player_id" in df.columns:
                    vals = pd.to_numeric(df.loc[df["player_id"].astype(str)==str(p), metric], errors="coerce")
                else:
                    vals = pd.to_numeric(df.loc[df["player_name"]==str(p), metric], errors="coerce")
                if vals.empty:
                    series.append(np.nan); continue
                found_any = True
                series.append(float(vals.mean() if agg=="mean" else vals.sum()))
            if found_any:
                Ys.append(np.array(series, dtype=float)); labels.append(str(p))
    
        seasons_sorted = sorted(set(seasons))
        title = f"{country} / {league}: Legendary Player Contributions over Seasons"
        subtitle = "Each line = one player; values per season (sum/mean as chosen)."
        fig = self.graphs.multiline(seasons_sorted, Ys, labels, title=title, subtitle=subtitle,
                                    figsize=(10,6), integer_xticks=True)
        return self._finish(fig, "legendary_career_compare")
    
    
    # ------------------------ Q21: TOP-5 LEAGUES SIDE-BY-SIDE -------------------------------
    
    def mod_top5_leagues_distribution(
        self, *,
        audit_df: pd.DataFrame,
        season: int,
        leagues: list[tuple[str,str]],     # [(country,league), ...] e.g., five major leagues
        contrib_col: str | None = None
    ):
        """
        Compare distributions across specified leagues for a given season via horizontal buckets.
        """
        import pandas as pd
        items = []
        for cc, lg in leagues:
            df = self._fetch_scope_df(country=cc, league=lg, season=season)
            if df.empty: continue
            metric = self._resolve_contrib_col(df.columns.tolist(), contrib_col)
            vals = pd.to_numeric(df[metric], errors="coerce").dropna().to_numpy(dtype=float)
            if vals.size == 0: continue
            items.append({"name": f"{lg}", "values": vals})
        if not items: raise ValueError("No league distributions to show.")
        title = f"{season}: Contribution Distributions — Cross-League"
        fig = self.graphs.bucket_horizontal(items, title=title, subtitle="", xlabel="Contribution",
                                            symmetric=True, showfliers=True)
        return self._finish(fig, "top5_leagues_distribution")
    
    
    # ------------------------ Q22/23: DOMESTIC vs FOREIGN (BY POSITION / TEAM) ---------------
    
    def mod_domestic_vs_foreign_by_position(
        self, *,
        country: str, league: str, season: int,
        contrib_col: str | None = None
    ):
        """
        Domestic vs Foreign comparison per position (grouped vertical buckets).
        Domestic defined by player_profiles.birth_country == country or analysis_results.country?
        We use analysis_results.country as league country; domestic if nationality == league country (fallback).
        """
        import pandas as pd, numpy as np
        ...
        # domestic flag: if 'nationality' exists else based on country field
        natcol = "nationality" if "nationality" in df.columns else None
        if natcol is None:
            # fallback: treat all as 'Unknown'; still produce structure
            df["_origin"] = "Unknown"
        else:
            df["_origin"] = np.where(df[natcol].astype(str)==str(country), "Domestic", "Foreign")
        ...
        for pos in ["Defenders","Midfielders","Strikers","Other"]:
            ...
            if sub.empty: continue
            data_map[pos] = {
                "Domestic": pd.to_numeric(sub.loc[sub["_origin"]=="Domestic", metric], errors="coerce").dropna().tolist(),
                "Foreign":  pd.to_numeric(sub.loc[sub["_origin"]=="Foreign",  metric], errors="coerce").dropna().tolist(),
            }
    
        title = f"{country} / {league} / {season}: Domestic vs Foreign by Position"
        fig = self.graphs.bucket_grouped_vertical(
            data_map, group_order=list(data_map.keys()),
            sub_order=["Domestic","Foreign"],
            title=title, subtitle="", ylabel="Contribution", show_legend=True
        )
        return self._finish(fig, "domestic_vs_foreign_by_position")
    
    
    def mod_team_best_by_nationality(
        self, *,
        country: str, league: str, season: int,
        contrib_col: str | None = None,
        target_nationality: str | None = None,    # if None, produce both Domestic & Foreign top per team (two figures)
        append_value_to_label: bool = True
    ):
        """
        For each team, label the best Domestic/Foreign (or specific nationality) player at the bar end (tornado).
        """
        import pandas as pd, numpy as np
        ...
        if "nationality" not in df.columns:
            raise ValueError("No 'nationality' column in analysis_results for this scope.")
    
        def _one_run(filter_func, label_caption):
            sub = df[filter_func(df)]
            if sub.empty:
                return None
            idx = sub.groupby(club_col, as_index=False)[metric].idxmax().dropna()[metric].astype(int).tolist()
            rows = sub.loc[idx]
            items = []
            for _, r in rows.iterrows():
                v = float(r[metric])
                nm = str(r[club_col])
                pl = str(r["player_name"])
                end = f"{pl} ({v:+.2f})" if append_value_to_label else pl
                items.append({"name": nm, "value": v,
                              "color": self.style.colors["top"] if v>=0 else self.style.colors["bottom"],
                              "end_text": end})
            items.sort(key=lambda d: d["value"], reverse=True)
            title = f"{country} / {league} / {season}: Best {label_caption} per Team"
            fig = self.graphs.tornado(items, title=title, subtitle="", xlabel="Contribution",
                                      symmetric=True, show_values=False)
            return self._finish(fig, f"best_{label_caption.lower()}_per_team")
    
        outs = []
        if target_nationality:
            outs.append(_one_run(lambda d: d["nationality"].astype(str)==str(target_nationality),
                                 label_caption=target_nationality))
        else:
            outs.append(_one_run(lambda d: d["nationality"].astype(str)==str(country), "Domestic"))
            outs.append(_one_run(lambda d: d["nationality"].astype(str)!=str(country), "Foreign"))
...
if __name__ == "__main__":
    ...
    # Load the audit once
    show.load_season_audit("season_audit.xlsx")  # stores on show.season_audit
    ...
    # Show only the best 12 clubs by contribution (largest to smallest)
    out = show.mod_team_top_player_tornado(
        country="Germany",
        league="Bundesliga",
        season=2023,
        top_n=12                      # keep first N after sorting by value
    )
    
    # # Append the numeric contribution to the end label: "Player (+0.42)"
    # out = show.mod_team_top_player_tornado(
    #     country="Germany",
    #     league="Bundesliga",
    #     season=2023,
    ...
    # Use a specific metric (defaults to contribution_ols if present)
    out = show.mod_team_top_player_tornado(
        country="Germany",
        league="Bundesliga",
        season=2023,
        contrib_col="ridge_contribution_alpha_1_0"
    )
    
    # # Keep left/right symmetry around zero (recommended for comparability)
    # out = show.mod_team_top_player_tornado(
    #     country="Germany",
    #     league="Bundesliga",
    #     season=2023,
    #     symmetric=True                # default True
    # )
    ...
    # Most common: one league-season scope
    out = show.mod_top5_lists(country="Germany", league="Bundesliga", season=2023, top_n=5)
    print("Saved:", out)
    
    # Specify a contribution column explicitly (if you prefer ridge)
    out = show.mod_top5_lists(country="Germany", league="Bundesliga", season=2023,
                              top_n=5, contrib_col="ridge_contribution_alpha_1_0")
    print("Saved:", out)
    
    # Apply a minutes filter to avoid cameo appearances
    out = show.mod_top5_lists(country="Germany", league="Bundesliga", season=2023,
                              top_n=5, min_minutes=900)
    
    
    
    """
    # Germany / Bundesliga / 2023
    out = show.mod_toplists_extended(
        audit_df=show.season_audit,   # used to resolve scope & future extensions
        country="Germany", league="Bundesliga", season=2023,
        top_n=5,
        include_overlap=True,
        include_winners_losers=True,
        include_most_improved=True,
        min_minutes=600
    )
    print(out)
    ...
    out = show.mod_bottom3_top3_contributors_table(
        country="Germany", league="Bundesliga", season=2023,
        min_minutes=600
    )
    print(out)
    
    out = show.mod_position_median_tornado(
        country="Germany", league="Bundesliga", season=2023,
        by="position"  # or any categorical field available in analysis_results
    )
    print(out)
    
    
    out = show.mod_position_buckets_horizontal_positions(
        country="Germany", league="Bundesliga", season=2023
    )
    print(out)
    
    
    out = show.mod_contrib_vs_age_clean(
        country="Germany", league="Bundesliga", season=2023,
        min_minutes=600
    )
    print(out)
    ...
    # Example: young (<=22) in the current Bundesliga whose previous season was in NL/CH or second tiers
    out = show.mod_young_development_secondary(
        audit_df=show.season_audit,
        main_country="Germany", main_league="Bundesliga", main_season=2023,
        age_max=22,
        secondary_filters={"country": ["Netherlands", "Switzerland", "Germany", "England", "Spain", "Italy", "France"]},
        max_players=20
    )
    ...
    # Provide a classifier for league names in your audit
    divisions_map = {
        "Bundesliga": "Top", "2. Bundesliga": "Second",
        "Premier League": "Top", "Championship": "Second",
        "La Liga": "Top", "Segunda División": "Second",
        # others default to "Other"
    }
    
    out = show.mod_division_distributions(
        audit_df=show.season_audit,
        season=2023,
        divisions_map=divisions_map,
        leagues=None  # or e.g., ["Bundesliga", "2. Bundesliga", "Premier League", ...]
    )
    print(out)
    
    
    out = show.mod_new_vs_existing_by_position(
        country="Germany", league="Bundesliga", season=2023
    )
    print(out)
    ...
    # Compare Thomas Müller vs other long-tenure players over selected seasons
    players = ["Thomas Müller", "Marco Reus", "Toni Kroos"]
    seasons = list(range(2014, 2024))  # last 10 seasons
    
    out = show.mod_legendary_player_career_compare(
        country="Germany", league="Bundesliga",
        players=players, seasons=seasons,
        use_ids=False,       # True if you prefer matching by player_id
        agg="sum",           # or "mean"
        club_filter=None     # e.g., "Bayern Munich" to isolate single-club tenure
    )
    print(out)
    ...
    out = show.mod_top5_leagues_distribution(
        audit_df=show.season_audit,
        season=2023,
        leagues=top5
    )
    ...
    # Domestic determined by nationality == league country (fallback if profiles not joined)
    out = show.mod_domestic_vs_foreign_by_position(
        country="Germany", league="Bundesliga", season=2023
    )
    print(out)
    ...
    # Domestic per team
    outs = show.mod_team_best_by_nationality(
        country="Germany", league="Bundesliga", season=2023,
        target_nationality=None,          # None => generate Domestic AND Foreign plots
        append_value_to_label=True
    )
    print(outs)
    
    # Or: best Argentines per team across the league
    outs = show.mod_team_best_by_nationality(
        country="Germany", league="Bundesliga", season=2023,
        target_nationality="Argentina",
        append_value_to_label=True
    )
    print(outs)
    
    
    # Default: all clubs, top contributor per club
    out = show.mod_team_top_player_tornado(
        country="Germany", league="Bundesliga", season=2023,
        append_value_to_label=True,   # "Player (+0.42)"
        symmetric=True
    )
    print(out)
    
    # Limit to top 12 clubs by contributor value
    out = show.mod_team_top_player_tornado(
        country="Germany", league="Bundesliga", season=2023,
        top_n=12, append_value_to_label=True
    )
    print(out)
    
    
    out = show.mod_team_ranking(
        country="Germany", league="Bundesliga", season=2023,
        sort_ascending=False,         # best at top
        annotate_top_player=True
    )
    print(out)
    
    
    # Existing API, single league; to compare across leagues, call twice or extend with show.audit
    out = show.mod_position_buckets_horizontal_positions(
        country="Germany", league="Bundesliga", season=2023
    )
    print(out)
    ...
    out = show.mod_contrib_vs_fte_multi(
        title_suffix="Contributions vs FTE",
        xlabel="FTE games (minutes/90)",
        ylabel="Contribution"
    )
    print(out)
    ...
    out = show.mod_contrib_vs_fte_by_nationality(
        top_n=10, include_other=True, min_minutes=180
    )
    print(out)
    
    
    # Prefer the "clean" version above; this is the original API
    out = show.mod_contrib_vs_age(by_position=True, min_minutes=180,
                                  anchor_age_min=30, breakout_age_max=22,
                                  add_regression=True, add_ci="prediction")
    print(out)
    
    
    out = show.mod_contrib_vs_fte_by_transfer_status(min_minutes=180, highlight_top_k_each=4)
    print(out)
    ...
    # Build comp_df from previous season first
    import sqlite3, pandas as pd
    with sqlite3.connect("analysis_results.db") as con:
        comp_df = pd.read_sql_query(
            "SELECT * FROM analysis_results WHERE country=? AND league=? AND season=?",
            con, params=["Germany","Bundesliga",2022]
        )
    
    out_improve = show.mod_season_improve(comp_df, max_items=30, sort_start_ascending=True)
    print(out_improve)
    
    out_decline = show.mod_season_decline(comp_df, max_items=30, sort_right_ascending=True)
    print(out_decline)
    
    
    out = show.mod_top5_lists(
        country="Germany", league="Bundesliga", season=2023,
        top_n=5, min_minutes=900     # filter cameos
    )
    print(out)
