...
class SeasonAnalyzer:    
    def __init__(
        ...
        date_cutoff: Optional[str] = None,  # NEW: optional YYYY-MM-DD (skip matches after this date)
    ):
        # Config
    ...
    #     Applies self.date_cutoff (if set): skips fixtures strictly after cutoff.
    #     """
    #     assert self.fixtures_data is not None and self.match_events is not None, "Call load_data() first."
    
    #     # Regular-season fixtures with data (respect cutoff)
    #     # build reg_fixture_ids
    #     reg_fixture_ids = []
    #     for fix in self.fixtures_data:
    #         if not self._is_regular(fix):
    #             continue
    #         fdate = self._to_utc(fix.get("fixture", {}).get("date"))
    #         if self.date_cutoff_utc is not None and fdate is not None and fdate > self.date_cutoff_utc:
    #             continue
    ...
    #         fid = str(fix["fixture"]["id"])
    #         fdate = self._to_utc(fix.get("fixture", {}).get("date"))
    #         if (self.date_cutoff_utc is not None and fdate is not None and fdate > self.date_cutoff_utc) or (fid not in self.valid_fixture_ids):
    #             continue
    ...
    #         game_date = fdate.isoformat() if fdate is not None else None
    ...
    #     """Per-player appearance counts and minutes played for valid fixtures (respects cutoff)."""
    ...
    #     """Create per-match summary and league table summary for valid fixtures (respects cutoff)."""
    ...
    #                 if mins == total_minutes and total_minutes > 0:
    #                     always_playing += 1
    ...
    #     """Add total goals and assists for each player (valid fixtures only, respects cutoff)."""
    ...
    def save_results(self, output_path: Optional[str] = None) -> None:
        """Save per-player and pooled results to Excel and SQLite; export DB slice."""
        assert self.coef_df_final is not None
        output_path = output_path or self._p("output.xlsx")
    
        # ---- Per-player: drop "p" prefix for output & DB
        ols_df = self.coef_df_final.copy()
        ols_df["player_id"] = ols_df["player_id"].apply(lambda pid: int(pid[1:]) if isinstance(pid, str) and pid.startswith("p") else pid)
        ols_df.sort_values(by="impact", ascending=False, inplace=True)
        ...
        if table_exists:
            ...
            new_cols = set(ols_df.columns)
            if existing_cols != new_cols:
                ols_df.to_sql("analysis_results_new", conn, if_exists="replace", index=False)
            else:
                cur.execute(
                ...
                conn.commit()
                ols_df.to_sql("analysis_results", conn, if_exists="append", index=False)
        else:
            ols_df.to_sql("analysis_results", conn, if_exists="replace", index=False)
    
        # Optional: store pooled results in separate table (string ids allowed)
        if self.coef_df_pooled is not None and not self.coef_df_pooled.empty:
            self.coef_df_pooled.to_sql("analysis_results_pooled", conn, if_exists="replace", index=False)
    
        conn.close()
    
        # Export the per-player slice from DB
    ...
    def run_with_cutoff(self, until_date: Optional[str] = None, group_pooled: bool = True) -> None:
