...
class OffBalanceShowcase:
    ...
    class Graphs:
        ...
        def scatter_fte(
            ...
            *,
            ...
            # Build a set of highlighted coordinates to avoid double plotting
            highlight_pts = set()
            if highlights:
                for h in highlights:
                    try:
            ...
            # Plot each source (skipping points that are highlighted)
            legend_labels = []
            for src in sources:
                ...
                # remove points that are also in highlights
                if highlight_pts:
                ...
                name   = src.get("name", None)            # legend label
                marker = src.get("marker", "o")
                size   = float(src.get("size", 40.0))
                alpha  = float(src.get("alpha", 0.70))
        
                ax.scatter(x, y, s=size, alpha=alpha, color=color, marker=marker, label=name, zorder=2)
                if name:
                    legend_labels.append(name)
        
            # Draw highlighted points + labels (no legend entries)
            if highlights:
                side2ha_va_offset = {
            ...
            if hline == "zero":
                ...
                # compute mean from all non-highlighted source points
                all_y = []
                for src in sources:
                    y = np.asarray(src.get("y", []), dtype=float)
                    x = np.asarray(src.get("x", []), dtype=float)
                    mask = np.isfinite(x) & np.isfinite(y)
                    y = y[mask]
                    if highlight_pts and y.size and x.size:
                        keep = np.array([(float(xi), float(yi)) not in highlight_pts for xi, yi in zip(x[mask], y)], dtype=bool)
                        y = y[keep]
                    if y.size:
                        all_y.append(y)
                if all_y:
                    ...
                    if np.isfinite(m):
                        ax.axhline(m, color="black", lw=0.8, linestyle="--", alpha=0.8, zorder=1)
        
            # Optional regression line (closed-form OLS to avoid SVD)
            if regline:
                Xs, Ys = [], []
                for src in sources:
                ...
                if Xs:
                    ...
                    if X.size >= 2:
                        ...
                        ybar = Y.mean()
                        Sxx  = np.sum((X - xbar)**2)
                        if Sxx > 1e-12:  # skip if all X ~ identical
                            Sxy  = np.sum((X - xbar)*(Y - ybar))
                            b1   = Sxy / Sxx
                            b0   = ybar - b1 * xbar
                            xs   = np.linspace(np.nanmin(X), np.nanmax(X), 200)
                            ys   = b0 + b1*xs
                            ax.plot(xs, ys, color="black", lw=1.4, alpha=0.9, zorder=3)
                            ...
                            if reg_ci in ("mean", "prediction") and X.size >= 3:
                                ...
                                # 95% critical value (normal approx to avoid SciPy)
                                tcrit = 1.96 if X.size >= 30 else 2.0
                                ax.fill_between(xs, ys - tcrit*se, ys + tcrit*se,
                                                color="black", alpha=0.08, edgecolor="none", zorder=2.5)
        
            # Axes labels
            if xlabel: ax.set_xlabel(xlabel, fontsize=self.s.label_fontsize)
            if ylabel: ax.set_ylabel(ylabel, fontsize=self.s.label_fontsize)
        
            ax.grid(True, alpha=self.s.grid_alpha, zorder=0)
        
            # Legend logic: only if asked AND there are >1 distinct non-empty labels
            if show_legend:
                uniq = [l for l in dict.fromkeys(legend_labels) if l]
                if len(uniq) > 1:
                    ncol = max(1, (len(uniq) + 4) // 5)  # new column every 5 labels
                    ax.legend(frameon=False, ncol=ncol)
        
            self.s.apply_footer(fig)
            return fig
        
        def tornado(self, items, title="", subtitle="", xlabel=None,
                    symmetric=True, figsize=(8,6), show_values=False):
            """
            Tornado chart archetype.
        
            Parameters
            ----------
            items : list of dicts with keys {"name","value","color"}
                Bars are drawn for each item. If show_values is True, numeric values
                will be shown just outside the bar (right for positive, left for negative).
            show_values : bool, default False
                Whether to display each bar's value at the end of the bar (outside).
            """
            ...
            ax.invert_yaxis()                   # highest item top
            ax.axvline(0, color="black", lw=0.8, zorder=1)
        
            # X-limits: symmetric by default; extend a bit if we need room for value labels
            if symmetric:
                lim_data = max(abs(min(vals) if vals else 0.0), abs(max(vals) if vals else 0.0))
                lim_data = lim_data if lim_data > 0 else 1.0  # avoid zero-span axis
                # span for labels; extend if we show values so they can sit outside bars
                span = 2.0 * lim_data
            ...
            # Recompute span from actual limits
            xmin, xmax = ax.get_xlim()
            span = xmax - xmin
            min_from_zero = 0.03 * span  # keep value labels away from the centered name label
            edge_pad      = 0.01 * span  # small push outside the bar end
        
            # Center the category label (name) at x=0 on each bar (with readable background)
            ...
            # Optional numeric value labels outside each bar, avoiding collisions with the center label
            if show_values:
                for i, it in enumerate(items):
                    v = float(it["value"])
                    # Format; tweak if you prefer fewer decimals
                    label = f"{v:.2f}"
        
                    if v >= 0:
                        # Ensure we are not too close to x=0 (to avoid overlap with centered name)
                        x_target = max(v + edge_pad, min_from_zero)
                        ha = "left"
                    else:
                        x_target = min(v - edge_pad, -min_from_zero)
                        ha = "right"
        
                    ax.text(x_target, i, label,
                            ha=ha, va="center",
                            fontsize=self.s.tick_fontsize, color="black",
                            bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.9),
            ...
            ax.grid(True, axis="x", alpha=self.s.grid_alpha)
            self.s.apply_footer(fig)
            return fig

        def ranking(self, categories, values, title="", subtitle="", figsize=(7,6)):
            fig, ax = plt.subplots(figsize=figsize)
            fig.subplots_adjust(bottom=self.s.footer_lift)  # extra space for footer/xlabel
            fig.suptitle(title, fontsize=self.s.title_fontsize, fontweight=self.s.title_fontweight)
            ax.set_title(subtitle, fontsize=self.s.subtitle_fontsize, pad=10)
            ax.barh(categories, values, color="steelblue")
            # ax.invert_yaxis()
            ax.axvline(0, color="black", lw=0.8)
            ax.set_xlabel("Off-Balance-Sheet Contribution", fontsize=self.s.label_fontsize)
        ...
        def slope(
            ...
            *,
            ...
            """
            Minimal slope chart:
              - Input = list of dicts (or dict-of-arrays) with keys:
                  start_value, final_value, label, flag_to_write_value, color, linewidth
                Defaults:
                  flag_to_write_value -> False
                  color       -> first color in the current Matplotlib cycle
                  linewidth   -> 1.8
              - Sorts by start_value (ascending/descending via sort_start_ascending).
              - If final_value > start_value: place label LEFT of start (right-aligned).
                If final_value < start_value: place label RIGHT of start (left-aligned).
              - If flag_to_write_value=True: write Δ=final-start (signed, up to 3 significant digits):
                   * to the RIGHT of the final if Δ > 0
                   * to the LEFT  of the final if Δ < 0
              - Draws an arrow start→final and a dot on the start value.
            """
        
            # ---------- defaults ----------
            try:
                _first_cycle_color = mpl.rcParams["axes.prop_cycle"].by_key()["color"][0]
            except Exception:
                _first_cycle_color = "tab:blue"
            _default_lw = 1.8
            _default_flag = False
        
            # ---------- normalize: list[dict] OR dict-of-arrays ----------
            def _broadcast(val, n):
                if isinstance(val, (list, tuple, np.ndarray, pd.Series)):
                    if len(val) == n:
                        return list(val)
                    # length mismatch → fall back to scalar broadcast
                return [val] * n
            ...
            if isinstance(items, dict):
                ...
                if n == 0:
                    raise ValueError("Empty items dict passed to slope().")
        
                flags = _broadcast(items.get("flag_to_write_value", _default_flag), n)
                colors = _broadcast(items.get("color", _first_cycle_color), n)
                lws    = _broadcast(items.get("linewidth", _default_lw), n)
        
                for i in range(n):
                    sv, fv = float(starts[i]), float(finals[i])
                    if not (np.isfinite(sv) and np.isfinite(fv)):  # skip bad rows
                        continue
                    norm_items.append({
                        "start_value": sv,
                        "final_value": fv,
                        ...
                        "color": colors[i] if colors[i] is not None else _first_cycle_color,
                ...
                for it in items:
                    ...
                    if not (np.isfinite(sv) and np.isfinite(fv)):
                        continue
                    norm_items.append({
                        "start_value": sv,
                        "final_value": fv,
                        "label": str(it.get("label", "")),
                        "flag_to_write_value": bool(it.get("flag_to_write_value", _default_flag)),
                        "color": it.get("color", _first_cycle_color) or _first_cycle_color,
                        "linewidth": float(it.get("linewidth", _default_lw)),
                    })
            ...
            # ---------- sort by start_value ----------
            norm_items.sort(key=lambda d: d["start_value"], reverse=not sort_start_ascending)
        
            # ---------- figure ----------
            n = len(norm_items)
            fig, ax = plt.subplots(figsize=figsize)
            fig.subplots_adjust(bottom=self.s.footer_lift)  # left=0.22, space for labels/footer
            fig.suptitle(title, fontsize=self.s.title_fontsize, fontweight="bold")
            ax.set_title(subtitle, fontsize=self.s.subtitle_fontsize, pad=10)
        
            # ---------- x-limits with padding for labels and Δ ----------
            starts = np.array([d["start_value"] for d in norm_items], dtype=float)
            finals = np.array([d["final_value"] for d in norm_items], dtype=float)
            X = np.r_[starts, finals]
            x_min = float(np.nanmin(X)); x_max = float(np.nanmax(X))
            if x_max <= x_min: x_max = x_min + 1.0
            span = x_max - x_min
        
            # Will we need left/right margin for labels?
            needs_left  = any(d["final_value"] > d["start_value"] for d in norm_items) \
                          or any(d["flag_to_write_value"] and (d["final_value"] - d["start_value"] < 0) for d in norm_items)
            needs_right = any(d["final_value"] < d["start_value"] for d in norm_items) \
                          or any(d["flag_to_write_value"] and (d["final_value"] - d["start_value"] > 0) for d in norm_items)
            ...
            # ax.set_ylim(-0.5, n - 0.5)  # <- fixes “infinite ribbon” images
            # AFTER — lift arrows slightly by adding padding in "row" units
            pad_bottom = 0.15   # increase to lift more
            pad_top    = 0.05
            ax.set_ylim(-0.5 - pad_bottom, n - 0.5 + pad_top)

        
            text_dx = 0.02 * (ax.get_xlim()[1] - ax.get_xlim()[0])
        
            # ---------- draw rows ----------
            for yi, d in enumerate(norm_items):
                ...
                # label placement relative to START:
                #   increase → left of start (right-aligned)
                #   decrease → right of start (left-aligned)
                if a > b:
                ...
                # optional Δ at final (side by sign)
                if d["flag_to_write_value"]:
                    # up to 3 significant digits, always show sign
                    try:
                    ...
                    ax.text(x_d, yi, delta_txt,
                            ha=ha_d, va="center",
                            fontsize=self.s.tick_fontsize, color=color,
        ...
        def bucket_horizontal(
            ...
            *,
            ...
            # --- new controls (safe defaults; no need to set) ---
            box_width: float = 0.55,         # vertical thickness of the thin boxplot boxes
            band_scale: float = 1.25,        # IQR "center band" height as a multiple of box_width
            band_alpha: float = 0.10,        # opacity of the IQR band (drawn under the label)
        ):
            """
            Tornado-like horizontal buckets: one horizontal box per 'name', order preserved.
        
            items: list of dicts:
              - "name":   category label (team, position bucket, etc.)
              - "values": sequence of numeric observations
              - "color":  optional face color for this category (default = style.colors["fill"])
        
            Renders:
              - Horizontal boxplot (25–75%) + whiskers + outliers.
              - A *thicker IQR band* (Q1↔Q3) so the bar remains visible even under the centered name label.
              - Vertical line at x = 0.
              - Category 'name' drawn at x = 0 (center), with white background (à la tornado).
            """
            import numpy as np
            import matplotlib as mpl
            ...
            names, data, colors = [], [], []
            default_face = self.s.colors.get("top", "orange") 
            ...
            for i, box in enumerate(bp["boxes"]):
                box.set_facecolor(colors[i]); box.set_alpha(0.50)
            ...
            for pair in cap_pairs:
                for c in pair:
                    c.set_color("black"); c.set_linewidth(1.0)
        
            # --- thick IQR band drawn *over* the thin box and *under* the label ---
            #     This makes the middle part visible even when the centered label overlaps.
            xspan = ax.get_xlim()[1] - ax.get_xlim()[0]
            band_h = min(0.95, float(box_width) * float(band_scale))  # keep within row spacing
            for y, vals, col in zip(pos, data, colors):
                if vals.size < 2:
                    # fallback: tiny band at median so something is visible
            ...
            for y, nm in zip(pos, names):
                ax.text(0, y, nm, ha="center", va="center",
                        fontsize=self.s.tick_fontsize,
                        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.18),
                        zorder=4)
        
            # top item at top (match tornado)
            ax.invert_yaxis()
        ...
        def bucket_grouped_vertical(
            ...
            *,
            ...
            Grouped vertical buckets:
              Top groups on x-axis (e.g., Defenders/Midfielders/Strikers).
              Within each group, identical sub-observations (e.g., BL‑2023 vs BL‑2024) appear side-by-side.
        
            data_map = {
              "Defenders":  {"BL-2023": [...], "BL-2024": [...]},
              "Midfielders":{"BL-2023": [...], "BL-2024": [...]},
              "Strikers":   {"BL-2023": [...], "BL-2024": [...]},
            }
        
            group_order: desired order for the top groups
            sub_order:   desired order for sub-groups (must be identical for all groups)
            """
            import numpy as np
            ...
            # palette for sub-groups
            palette = self._parent_owner.default_colors if hasattr(self, "_parent_owner") else [
            ...
            # layout: blocks of nS boxes with a blank column between groups
            gap = 1.0
            width = 0.65
            positions = []
            box_data  = []
            ...
            for g in group_order:
                ...
                for j, sg in enumerate(sub_order):
                    vals = np.asarray(list((data_map.get(g, {}) or {}).get(sg, [])), dtype=float)
                    vals = vals[np.isfinite(vals)]
                    if vals.size == 0:
                        # keep placeholder to preserve alignment
        ...
        def multiline(self, x, ys, labels, title="", subtitle="", figsize=(8,6), integer_xticks=False):
            import numpy as np
            ...
            fig, ax = plt.subplots(figsize=figsize)
            fig.subplots_adjust(bottom=self.s.footer_lift)  # space for footer/xlabel
            fig.suptitle(title, fontsize=self.s.title_fontsize, fontweight=self.s.title_fontweight)
            ax.set_title(subtitle, fontsize=self.s.subtitle_fontsize, pad=10)
            ...
            ax.set_xlabel("Season / Age", fontsize=self.s.label_fontsize)
            ax.set_ylabel("Off-Balance-Sheet Contribution", fontsize=self.s.label_fontsize)
            ax.legend(frameon=False, fontsize=self.s.legend_fontsize)
            ax.grid(True, alpha=self.s.grid_alpha)
        
            if integer_xticks:
                # If x values are integer-like, enforce integer ticks exactly at those x's
                unique_ints = sorted({int(round(v)) for v in x if np.isfinite(v)})
                if unique_ints:
                    ax.xaxis.set_major_locator(FixedLocator(unique_ints))
                else:
        ...
        def small_multiples_lines(
            ...
            *,
            ...
            Small multiples (lines):
              - Each panel shows one subject's time series (solid with markers).
              - Faint background lines (others) provide context.
              - Allowed N panels: 2, 3, 4, 6, 8 (else raise).
            """
            import numpy as np
            ...
            for p in panels:
                ...
                colors.append(p.get("color", None))
        
            # global y-limits for comparability
            allY = np.concatenate([np.asarray(y, dtype=float) for y in Ys if len(y) > 0])
            ...
            # pre-plot all background lines into each panel
            for ax in axes[:N]:
                for y in Ys:
                    ax.plot(x, y, alpha=0.15, linewidth=1.2, color="gray")
        
            # then draw the focal subject on each panel
            for ax, y, t, c in zip(axes[:N], Ys, titles, colors):
                ...
                # integer ticks if x is integer-like
                unique_ints = sorted({int(round(v)) for v in x if np.isfinite(v)})
                if unique_ints:
                    ax.xaxis.set_major_locator(FixedLocator(unique_ints))
                ax.grid(True, alpha=self.s.grid_alpha)
        ...
        def small_multiples(self, dfs, titles, title="", subtitle="", figsize=(12,8)):
            n = len(dfs)
            ...
            for ax,df,t in zip(axes,dfs,titles):
                ...
                ax.set_title(t, fontsize=self.s.subtitle_fontsize)
                ax.grid(True, alpha=self.s.grid_alpha)
            self.s.apply_footer(fig)
            return fig

        def lorenz(self, values, title="", subtitle="", figsize=(6,6)):
            vals = np.sort(values)
    ...
    def __init__(self, *,
        ...
        self.graphs = OffBalanceShowcase.Graphs(self.style)
        self.graphs._parent_owner = self
    
        # data holders
        self.players_df = pd.DataFrame()
        self.players_df_comp = pd.DataFrame()
    
        # scope/context (for titles); set via .load(...) or .set_context(...)
    ...
    def _fmt_scope_title(self, fallback: str = "") -> str:
        """
        Build a compact title prefix from context.
        Examples: 'Germany Bundesliga 2023', 'Ligue 1 2024', '2023', or ''.
        """
        parts = []
    ...
    def set_players(self, df: pd.DataFrame, *, context_label: str | None = None) -> None:
        """
        Directly set the working DataFrame and optional label for titles.
        """
    ...
    def _parse_date_safe(self, s):
        if s is None:
            return pd.NaT
        return pd.to_datetime(str(s), errors="coerce", utc=True)
    ...
    def _finish(self, fig, name: str):
        out = os.path.join(f"{self.country}_{self.league}_{self.season}_{name}.png")
        if self.save_plot:
    ...
    # ):
    #     """
    #     Small multiples: per-team scatter (FTE vs contribution) for the current season.
    #       - Picks top-N teams by median (or mean) contribution.
    #       - Each panel shows team players as dots.
    #       - Y axis is shared across panels for fair comparison.
    #     """
    #     import numpy as np
    #     import pandas as pd
    ...
    #     # FTE
    #     if "FTE_games_played" in df.columns:
    #         df["FTE"] = pd.to_numeric(df["FTE_games_played"], errors="coerce").clip(lower=0).fillna(0.0)
    #     else:
    #         df["FTE"] = pd.to_numeric(df.get("minutes_played", 0), errors="coerce").fillna(0.0) / 90.0
    #         df["FTE"] = df["FTE"].clip(lower=0)
    
    #     if min_minutes > 0 and "minutes_played" in df.columns:
    #         df = df[pd.to_numeric(df["minutes_played"], errors="coerce").fillna(0.0) >= float(min_minutes)]
    
    #     # exclude pseudo-teams
    #     df = df[~df["team(s)"].fillna("").str.startswith("Multiple Teams")]
    #     df = df.dropna(subset=[self._imp_col, "team(s)"])
    
    #     # compute team ranking metric
    #     def _metric(g: pd.DataFrame) -> float:
    #         vals = pd.to_numeric(g[self._imp_col], errors="coerce")
    #         if order_by == "mean": return float(vals.mean())
    #         return float(vals.median())
    
    #     teams = []
    #     for tm, sub in df.groupby("team(s)"):
    #         if len(sub) >= int(min_players_per_team):
    #             teams.append((tm, _metric(sub)))
    
    #     if not teams:
    #         raise ValueError("No teams with sufficient players to plot.")
    
    #     teams_sorted = [t for t, _ in sorted(teams, key=lambda x: x[1], reverse=True)][:int(top_n)]
    
    #     # Build list of per-team DataFrames with columns x, y
    #     dfs, titles = [], []
    ...
    #             continue
    #         d = pd.DataFrame({
    #             "x": pd.to_numeric(sub["FTE"], errors="coerce"),
    #             "y": pd.to_numeric(sub[self._imp_col], errors="coerce")
    #         }).dropna()
    #         if d.empty:
    #             continue
    #         dfs.append(d)
    ...
    #     if not dfs:
    #         raise ValueError("No team panels to plot (after cleaning).")
    
    #     # dynamic width to keep panels readable
    #     fig_w = max(10, 4 * len(dfs))
    #     title = f"{self._fmt_scope_title('Scope')}: Top {len(dfs)} Teams — FTE vs Contribution"
    #     subtitle = "Each panel = one team; dots are players."
    #     fig = self.graphs.small_multiples(dfs, titles, title=title, subtitle=subtitle, figsize=(fig_w, 6))
    #     return self._finish(fig, "small_multiples_top_teams_scatter")

    # def mod_small_multiples_seasons_fte(
    #     self,
    #     seasons: list[int] | None = None,   # None → [season-2, season-1, season]
    #     *,
    #     min_minutes: float = 0.0,
    # ):
    #     """
    #     Small multiples: one panel per season (league-level FTE vs contribution).
    #       - Good for comparing distribution shifts across years.
    #     """
    #     import numpy as np
    #     import pandas as pd
    ...
    #         if self.season is None:
    #             raise ValueError("Provide seasons=... or set context with set_context(..., season=...).")
    #         seasons = [int(self.season) - 2, int(self.season) - 1, int(self.season)]
    #     seasons = [int(s) for s in seasons]
    
    #     dfs, titles = [], []
    #     for s in seasons:
    #         df = self.fetch_scope(country=self.country, league=self.league, season=s)
    #         if df.empty:
    #             continue
    ...
    #         d = pd.DataFrame({
    #             "x": pd.to_numeric(df["FTE"], errors="coerce"),
    #             "y": pd.to_numeric(df[self._imp_col], errors="coerce"),
    #         }).dropna()
    ...
    #     if not dfs:
    #         raise ValueError("No non-empty season panels to plot.")
    
    #     fig_w = max(12, 4 * len(dfs))
    #     title = f"{self._fmt_scope_title('Scope')}: FTE vs Contribution Across Seasons"
    #     subtitle = "Each panel = one season; dots are players."
    #     fig = self.graphs.small_multiples(dfs, titles, title=title, subtitle=subtitle, figsize=(fig_w, 6))
    #     return self._finish(fig, "small_multiples_seasons_fte")
...
if __name__ == "__main__":
    ...
    # out3 = show.mod_team_buckets_horizontal()
"""
    
    # out = show.mod_multiline_top_players_over_seasons(top_k=8, seasons=None, agg="sum")
    # Small multiples: top 6 teams by median contribution
    # out = show.mod_small_multiples_top_teams_scatter(top_n=6, min_players_per_team=8, order_by="median")
    # out = show.mod_small_multiples_seasons_fte()
