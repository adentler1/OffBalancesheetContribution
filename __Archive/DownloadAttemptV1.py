...
REQUEST_LIMIT_PER_DAY = 100         # Max requests per day (free tier)
TIME_BETWEEN_REQUESTS = 10          # Seconds to wait between API calls (rate-limit buffer)
BATCH_SIZE = 50                     # Number of fixtures per batch call
...
requests_remaining = REQUEST_LIMIT_PER_DAY

#%%
# 🔧 Helper Functions
def api_get(endpoint, params=None):
    """
    Perform GET with global rate-limit tracking.
    ...
    requests_remaining = int(resp.headers.get(
        'x-ratelimit-requests-remaining', requests_remaining - 1
    ))
    return resp.json()


def load_json(filename):
    """Load JSON from SAVE_DIR if exists, else return empty dict."""
    path = os.path.join(SAVE_DIR, filename)
    return json.load(open(path)) if os.path.exists(path) else {}


def save_json(filename, data):
    """Save JSON to SAVE_DIR."""
    path = os.path.join(SAVE_DIR, filename)
...
league_data = load_json(LEAGUE_FILE)
if not league_data:
    league_data = api_get('/leagues', {'name': LEAGUE_NAME, 'country': COUNTRY_NAME})
    save_json(LEAGUE_FILE, league_data)
...
for fid in fixture_ids:
    ...
    print(len(match_events[key]['lineups']))
