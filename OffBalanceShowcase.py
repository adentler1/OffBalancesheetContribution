...
class OffBalanceShowcase:
    ...
    def __init__(self, *,
                 ...
                 partialled: bool = False,              # NEW: toggle "partialled_" prefix
                 save_plot: bool = True,
        ...
        self.partialled = bool(partialled)             # NEW
        ...
        self._imp_col = "contribution_ols"             # will be resolved in _resolve_imp_col()
    ...
    def _resolve_imp_col(self, df: pd.DataFrame) -> str:
        """
        Choose the contribution column based on estimator/alpha and the `partialled` flag.
        Rules:
          - OLS → (partialled_)?contribution_ols
          - Ridge/Lasso with alpha is not None → (partialled_)?{est}_contribution_alpha_{a_str}
          - Ridge/Lasso with alpha is None → (partialled_)?{est}_contribution_alpha_best
        """
        cols = set(df.columns)
        pfx = "partialled_" if self.partialled else ""
    
        def a_to_str(a: float) -> str:
            # Normalize like "1.0" -> "1_0", "0.01" -> "0_01"
            return str(float(a)).replace(".", "_")
    
        if self.estimator == "ols":
            key = f"{pfx}contribution_ols"
            if key not in cols:
        ...
        if self.estimator == "ridge":
            if self.ridge_alpha is None:
                key = f"{pfx}ridge_contribution_alpha_best"
                if key in cols:
                    return key
                # graceful fallback: first ridge contribution with/without partialled prefix
                cands = [c for c in cols if c.startswith(f"{pfx}ridge_contribution_alpha_")]
                if cands:
                    return sorted(cands)[0]
                raise ValueError("No ridge contribution columns found.")
            else:
                key = f"{pfx}ridge_contribution_alpha_{a_to_str(float(self.ridge_alpha))}"
                if key not in cols:
                    raise ValueError(f"Column '{key}' not found for ridge_alpha={self.ridge_alpha}.")
                return key
    
        if self.estimator == "lasso":
            if self.lasso_alpha is None:
                key = f"{pfx}lasso_contribution_alpha_best"
                if key in cols:
                    return key
                cands = [c for c in cols if c.startswith(f"{pfx}lasso_contribution_alpha_")]
                if cands:
                    return sorted(cands)[0]
                raise ValueError("No lasso contribution columns found.")
            else:
                key = f"{pfx}lasso_contribution_alpha_{a_to_str(float(self.lasso_alpha))}"
                if key not in cols:
                    raise ValueError(f"Column '{key}' not found for lasso_alpha={self.lasso_alpha}.")
                return key
    ...
    def _resolve_contrib_col(self, cols: list[str], override: str | None = None) -> str:
        """
        Generic column chooser for 'contribution' metrics (used by modules that don't care about estimator).
        Honors `self.partialled`.
        Preference order:
          1) override (if present)
          2) (partialled_)?contribution_ols
          3) (partialled_)?ridge_contribution_alpha_best
          4) (partialled_)?lasso_contribution_alpha_best
          5) Any (partialled_)?{ridge,lasso}_contribution_alpha_*
          6) Any column containing 'contribution' (as last resort)
        """
        s = set(cols)
        if override and override in s:
            return override
    
        pfx = "partialled_" if self.partialled else ""
        prefs = [
            f"{pfx}contribution_ols",
            f"{pfx}ridge_contribution_alpha_best",
            f"{pfx}lasso_contribution_alpha_best",
        ]
        for c in prefs:
            if c in s:
                return c
    
        # any alpha-specific ridge/lasso with the chosen prefix
        alpha_cands = [c for c in cols if c.startswith(f"{pfx}ridge_contribution_alpha_")
                                      or c.startswith(f"{pfx}lasso_contribution_alpha_")]
        if alpha_cands:
            return sorted(alpha_cands)[0]
    
        # very last resort: any 'contribution' column at all (preferring partialled if available)
        any_partialled = [c for c in cols if c.startswith("partialled_") and "contribution" in c]
        if any_partialled:
            return sorted(any_partialled)[0]
        for c in cols:
            if "contribution" in c.lower():
                return c
    
        raise ValueError("No contribution-like column found.")
    ...
    def mod_team_top_player_tornado(
        ...
        *,
        ...
        metric = self._resolve_contrib_col(df.columns.tolist(), override=contrib_col)
    ...
    def mod_top5_lists(
        ...
        *,
        ...
        # Resolve contribution column (honors self.partialled and '*_alpha_best')
        contrib_col = self._resolve_contrib_col(df.columns.tolist(), override=contrib_col)
...
if __name__ == "__main__":
    ...
    show = OffBalanceShowcase(
        ...
        estimator="ridge", ridge_alpha=None,
